...
var config = {
    ...
    colors: ['#8CD6FF', 'white', 'white', 'white', 'white', 'white'],
...
function drawToolTipSplitLine(offsetX, opts, config, context) {
    ...
    context.beginPath();
    context.setStrokeStyle('#8CD6FF'); //Tip线颜色
    context.setLineWidth(1);
...
function drawToolTip(textList, offset, opts, config, context) {
    if (!config.is_number) {
        for (let i in config.yValue) {
            if (textList[0].text == config.yValue[i]) {
                textList[0].text = handleYTitle(config.yTitle[i]);
                break;
            }
        }
    }
...
var Charts = function Charts(opts) {
    ...
    this.scrollOption = {
        currentOffset: opts._scrollDistance_,
        startTouchX: 0,
        distance: 0
    };
