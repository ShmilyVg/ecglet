...
Component({
    ...
    methods: {
        ...
        drawWaveDark(data) {
            let that = this
            const ecgData = that.data, ctx = ecgData.ctx;
            ...
            let opx = ecgData.lastPoint.x, opy = ecgData.lastPoint.y || ecgData.baseY;
            let px = opx;
            let py = opy;
            let ogn = ecgData.lastPoint.adGain || buffer[0];
            ctx.beginPath();
            ctx.moveTo(opx, opy);
            ...
            buffer.forEach((gain, index) => {
                ...
                if (index === (len - 1)) {
                    ecgData.lastPoint.index += index;
                    ecgData.lastPoint.adGain = ogn;
                    ecgData.lastPoint.x = px;
                    ecgData.lastPoint.y = py;
                }
            });
